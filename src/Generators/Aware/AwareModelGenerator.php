...
use InfyOm\Generator\Generators\Aware\AwareSwaggerGenerator;

class AwareModelGenerator extends BaseGenerator
{
    /** @var CommandData */
    private $commandData;
    ...
    {
        $fieldTypes = AwareSwaggerGenerator::generateTypes($this->commandData->inputFields);

        $template = TemplateUtil::getTemplate('swagger.model.model', 'swagger-generator');
        ...
        $propertyTemplate = TemplateUtil::getTemplate('swagger.model.property', 'swagger-generator');

        $properties = AwareSwaggerGenerator::preparePropertyFields($propertyTemplate, $fieldTypes);
        ...
        foreach ($this->commandData->inputFields as $field) {
            if (!empty($field['validations'])) {
                if (str_contains($field['validations'], 'required')) {
                    $requiredFields[] = '"' . $field['fieldName'] . '"'; // AWARE-ALTERATION
                }
            }
        }
